...
socket.emit('error', { message: 'Failed to process guess' });
  ...
  socket.on('voting-results', async (data) => {
    ...
    try {
      ...
      // Generate new speaking order for remaining players
      const remainingPlayers = gameRoom.players.filter(p => !p.isEliminated);
      let newSpeakingOrder = remainingPlayers.map(p => p.userId.toString());
      newSpeakingOrder = newSpeakingOrder.sort(() => Math.random() - 0.5);

      // Update the current round with new speaking order
      const currentRound = gameRoom.rounds[gameRoom.currentRound - 1];
      if (currentRound) {
        currentRound.speakingOrder = newSpeakingOrder;
        currentRound.playerTurn = newSpeakingOrder[0];
      }
      ...
      io.to(roomCode).emit('phase-change', {
        phase: 'discussion',
        message: `Player ${eliminatedPlayer} has been eliminated. New round begins.`
      });
