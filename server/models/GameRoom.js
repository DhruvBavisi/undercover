...
const gameRoomSchema = new mongoose.Schema({
  ...
  players: [{
    ...
    role: {
      type: String,
      enum: ['civilian', 'undercover', 'mrwhite', 'spectator', ''],
      default: ''
    },
    word: {
      type: String,
  ...
  messages: [{
    ...
    isRoundHeader: { type: Boolean, default: false },
    round: Number,
    type: { type: String },
    timestamp: {
      type: Date,
...
gameRoomSchema.methods.addPlayer = function (player) {
  // If player is marked as eliminated initially, they are joining as a spectator, so we don't enforce maxPlayers
  const isJoiningAsSpectator = player.isEliminated === true;

  if (!this.hasPlayer(player.userId) && (isJoiningAsSpectator || !this.isFull())) {
...
gameRoomSchema.methods.startGame = function () {
  ...
  // Add the initial round header message to the cleared array
  this.messages.push({
    id: `round-header-1-${Date.now()}`,
    playerName: 'System',
